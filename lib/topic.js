...
exports.page = function(response, queryData) {
    ...
    db.query(`SELECT * FROM topic`, function(error, topics) {
        ...
        /**
         *  -> ? 로 queryData.id가 formatting 되는데 이 때 공격받는 코드를 
         *      처리해준다. url 뒤에 drop 과 같은 sql 명령을 붙어서 보내면 
         *      공격을 받을 수 있는데 아래와 같이 포맷팅해서 id 값을 넣어주면 그 부분이
         *      문자열 처리가 된다.
         * 
         *      또한
         * 
         *  (2) db.query는 한 번에 여러 개의 sql 문이 동작하지 않도록 되어있다.
         * 
         *  -> db.js에서 multipleStatements : true이면 여러 sql 문이 동작하게된다.
         * 
         *  (3) db.esacpe를 이용해서 탈출도 가능하다.
         *  */
        db.query(`SELECT * FROM topic LEFT JOIN author ON topic.author_id=author.id WHERE topic.id=?`, [queryData.id], function(error2, topic) {
            ...
            var html = template.HTML(title, list,
                `<h2>${title}</h2>
                        ...
                        <p>by ${topic[0].name}</p>
                        `,
                `<a href="/create">Create content</a><br>
                 <a href="/update?id=${queryData.id}">Update content</a>
